...
import rnn_model
import cnn_model
from dataloader_use import Dataloader
import os
import datetime
import numpy as np
import argparse
...
def main():
    ...
    probabilities, targets, observations, idobs = evaluate(model,dataloader,
    ...
    np.save(os.path.join(args.rundir, "eval_idobs.npy"), idobs)
...
def evaluate_rnn(model,
    ...
    with tf.Session(config=config) as sess:
        ...
        for i in range(1, dataloader.num_batches):
            ...
            X, y, seq_lengths, idobs = dataloader.next_batch()
            ...
            all_idobs =np.append(all_idobs, idobs) 
            #total_cm += cm
